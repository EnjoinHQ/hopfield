...
[ethers.js](https://github.com/ethers-io/ethers.js) is a JavaScript library for interacting with Ethereum. Among other features, it has utilities for working with human-readable ABIs. In addition to runtime functions, Hopfield also has type utilities for working with [human-readable ABIs](/api/human).
...
- Hopfield returns inferred ABI, while ethers.js just returns `readonly Fragment[]`
- Hopfield supports [structs](/api/human.html#structs) as signatures and inline tuples (e.g. `(string name, uint256 age)`).
- ethers.js does not support struct signatures, but does support inline tuples with the optional `tuple` prefix keyword.
- Hopfield supports mixed named and unnamed parameters, while ethers.js only supports either all named or all unnamed parameters.
...
```ts twoslash [hopfield]
import { parseAbiItem } from 'hopfield'
...
- Hopfield returns inferred ABI item, while ethers.js just returns `Fragment`
...
· hopfield   597,815.38  0.0015  0.3760  0.0017  0.0016  0.0022  0.0022  0.0031  ±0.58%   298908   fastest
...
Summary
hopfield - src/human-readable/parseAbiItem.bench.ts > comparison
...
```ts twoslash [hopfield]
import { parseAbiParameter } from 'hopfield'
...
- Hopfield returns inferred ABI parameter, while ethers.js just returns `ParamType`
- Rest same as [`Interface` versus `parseAbi`](#interface-versus-parseabi)
...
✓ Parse basic ABI Parameter (3) 2358ms
  name                hz     min     max    mean     p75     p99    p995    p999     rme  samples
· hopfield   4,073,274.42  0.0001  1.0815  0.0002  0.0003  0.0003  0.0003  0.0005  ±0.52%  2036638   fastest
...
Summary
hopfield - src/human-readable/parseAbiParameter.bench.ts > Parse basic ABI Parameter
...
```ts twoslash [hopfield]
import { formatAbi } from 'hopfield'
...
Hopfield returns inferred human-readable ABI, while ethers.js just returns `string[]`.
...
✓ Format ABI (3) 2068ms
· hopfield   1,687,145.72 ops/sec ±0.39% (843573 samples) fastest
· ethers@5    117,287.14 ops/sec ±0.36% ( 58644 samples)
· ethers@6     46,075.42 ops/sec ±0.37% ( 23038 samples) slowest

Summary
hopfield - src/human-readable/formatAbi.bench.ts > Format ABI
...
```ts twoslash [hopfield]
import { formatAbiItem } from 'hopfield'
...
Hopfield returns inferred human-readable ABI item, while ethers.js just returns `string`.
...
· hopfield   4,833,836.91 ops/sec ±0.89% (2416919 samples) fastest
· ethers@6    123,697.64 ops/sec ±0.32% (  61849 samples) slowest
· ethers@5    343,959.66 ops/sec ±0.33% ( 171980 samples)

Summary
hopfield - src/human-readable/formatAbiItem.bench.ts > Format basic ABI function
...
```ts twoslash [hopfield]
import { formatAbiParameter } from 'hopfield'
...
Hopfield returns inferred human-readable ABI parameter, while ethers.js just returns `string`
...
· hopfield   10,550,231.55 ops/sec ±0.63% (5275116 samples) fastest
· ethers@6     398,639.32 ops/sec ±0.40% ( 199320 samples) slowest
· ethers@5   1,041,080.97 ops/sec ±0.45% ( 520541 samples)

Summary
hopfield - src/human-readable/formatAbiParameter.bench.ts > Format basic ABI Parameter
